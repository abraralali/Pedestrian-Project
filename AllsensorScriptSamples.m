...
%                  s = minus(max(segment.timestamp),min(segment.timestamp));
%              end
             Pgt = groundTruth(groundTruth.timestamp <= accel.timestamp(i),:);
             threshold=1; % 1m/s2
%              sigStd(j)=std(accel.z);
             if accel.z(i)<=threshold %this segment is standing
                PwaitingTime = PwaitingTime + s;
                pstandtime = [pstandtime;accel.timestamp(i)];
                plot(seconds(accel.timestamp(i)-starttime),signal(i),'color','r');%standing
                hold on
                standingTimestamps = [standingTimestamps;accel.timestamp(i)];
%                  for q=1:length(segment.timestamp)
                   dist = datenum(accel.timestamp(i)) - datenum(Pgt.timestamp);
                   [~,qq] = min(dist);
%                  end
                Standingsegment = [Standingsegment; {'STAND'}, Pgt.tag(qq)];
             else 
                plot(seconds(accel.timestamp(i)-starttime),accel.z,'color','g');%walking
                walkingTimestamps = [walkingTimestamps;accel.timestamp(i)];
                hold on
                pwalktime = [pwalktime;accel.timestamp(i)];
%                 for q=1:length(segment.timestamp)
                   dist = datenum(accel.timestamp(i)) - datenum(Pgt.timestamp);
                   [~,qq] = min(dist);
%                 end
